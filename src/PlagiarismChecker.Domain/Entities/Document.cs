...
public sealed class Document
{
    ...
    public static Document Create(
        long[] documentSortedWordHashes,
        long[] numericOrderedWordHashes,
        int[] numericOrderedWordIndexes,
        int firstWordIndex
    )
    {
        return new Document
        {
            Id = DocumentId.New(),
            DocumentSortedWordHashes = documentSortedWordHashes,
            NumericOrderedWordHashes = numericOrderedWordHashes,
            NumericOrderedWordIndexes = numericOrderedWordIndexes,
            FirstWordIndex = firstWordIndex,
            WordsCount = documentSortedWordHashes.Length
        };
    }

    public DocumentId Id { get; private set; }
    public long[] DocumentSortedWordHashes { get; private set; }
    public long[] NumericOrderedWordHashes { get; private set; }
    public int[] NumericOrderedWordIndexes { get; private set; }
    ...
    )
    {
        ...
        var wordNumberL = docL.FirstWordIndex; // start left at first >3 letter word
        // word number for left document and right document
        var wordNumberR = docR.FirstWordIndex;
                     ...
                     iWordNumberR++) // loop for each copy of this word on the right
                {
                    ...
                    if (comparisonParameters.MismatchTolerance > 0) // are we accepting imperfect matches?
                    {
                        ...
                        while (firstL >= 0 && firstR >= 0) // if we aren't at the start of either document,
                        {
                            ...
                            flaws++;
                            if (flaws > comparisonParameters.MismatchTolerance)
                                break; // check for maximum flaws reached
                               ...
                               lastR < docR.WordsCount) // if we aren't at the end of either document
                        {
                            ...
                            if (lastR + 1 < docR.WordsCount) // check one word later on right (if it exists)
                            {
                                ...
                                if (docL.DocumentSortedWordHashes[lastL] == docR.DocumentSortedWordHashes[lastR + 1])
                                {
                                    ...
                                    if (matching < comparisonParameters
                                            .MismatchPercentage) // are we getting too imperfect?
                                    {
                                        break;
                                    }
